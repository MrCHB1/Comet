...
void ColorPaletteList::ValidateColorPaletteList() {
  ...
  if (numColors == 0)
    CreateDefaultColorPalettes(false);
}
  ...
  // default rainbow (8 channels)
  {
    ...
    for (int track = 0; track < 8; track++) {
      for (int channel = 0; channel < 16; channel++) {
        ...
        ImGui::ColorConvertHSVtoRGB((float)colorIdx / 8.0f, 1.0f, 1.0f, r, g,
                                    b);
  ...
  // 16-color rainbow
  {
    ...
    for (int track = 0; track < 16; track++) {
      for (int channel = 0; channel < 16; channel++) {
        ...
        ImGui::ColorConvertHSVtoRGB((float)colorIdx / 16.0f, 1.0f, 1.0f, r, g,
                                    b);
    ...
    stbi_write_png("./colors/Rainbow16.png", 16, 16, 3, colorData.data(),
                   16 * 3);
  }
}

ColorPaletteEntry
ColorPaletteList::LoadPaletteFromImage(const std::filesystem::path &path) {
